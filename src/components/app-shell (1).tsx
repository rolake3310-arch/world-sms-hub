...
import { LayoutDashboard, Send, Wallet, History, Shield, LogOut, MessageSquare, Smartphone, Settings } from "lucide-react";
    ...
    <div className="min-h-screen bg-gradient-surface">
      ...
      <header className="sticky top-0 z-10 flex items-center justify-between border-b border-border bg-background/80 px-4 py-3 backdrop-blur md:hidden">
        ...
        <div className="flex items-center gap-3">
          <div className="text-sm font-semibold tabular-nums">{fmt(balance)}</div>
...
function MobileTabs({ isAdmin }: { isAdmin: boolean }) {
  ...
  const pathname = useRouterState({ select: (s) => s.location.pathname });
  return (
    <nav className="fixed inset-x-0 bottom-0 z-20 grid border-t border-border bg-background/90 backdrop-blur md:hidden"

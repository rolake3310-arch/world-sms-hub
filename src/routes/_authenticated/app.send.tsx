...
function SendPage() {
  ...
  const mutation = useMutation({
    mutationFn: (vars: { sender: string; recipients: string[]; message: string }) =>
      sendFn({ data: vars }),
    onSuccess: (r) => {
      toast.success(`Sent ${r.sent} message(s) for $${r.total.toFixed(4)}`);
      setRecipientsRaw("");
      setMessage("");
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
        <Card className="space-y-4 p-5">
          ...
          >
            ...
            {mutation.isPending ? "Sending..." : insufficient ? "Insufficient balance" : `Send for $${(quote?.total ?? 0).toFixed(4)}`}
          </Button>
        </Card>
        ...
        <Card className="space-y-3 p-5">
          ...
          <div className="rounded-lg bg-secondary p-3">
            ...
            <div className="text-2xl font-bold tabular-nums">${(quote?.total ?? 0).toFixed(4)}</div>
                ...
                <tbody>
                  {quote.lines.map((l, i) => (
                    <tr key={i} className="border-t border-border">
                      ...
                      <td className="p-2 text-right tabular-nums">${l.cost.toFixed(4)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>

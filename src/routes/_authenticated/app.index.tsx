  ...
  return (
    <div className="space-y-6 pb-24 md:pb-0">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <StatCard label="Wallet Balance" value={`$${balance.toFixed(2)}`} icon={Wallet} accent />
        <StatCard label="Messages Sent" value={String(sent)} icon={MessageSquare} />
        <StatCard label="Total Spent" value={`$${spent.toFixed(2)}`} icon={TrendingUp} />
      </div>
            ...
            <ul className="divide-y divide-border">
              {(deps ?? []).slice(0, 5).map((d) => (
                <li key={d.id} className="flex items-center justify-between py-2 text-sm">
                  <div>
                    <div className="font-medium">${Number(d.amount_usd).toFixed(2)} <span className="text-xs text-muted-foreground">via {d.method}</span></div>
